...
def auto_trap_pr(owner: str, repo: str, req: AutoTrapPRRequest):
    ...
    try:
        ...
        # （既存の pr = repository.create_pull(...) のすぐ下あたりに追記）

        # ----------------------------------------------------
        # STEP 4: Firestoreに罠PRの情報を記録（新規追加）
        # ----------------------------------------------------
        # ドキュメントIDを一意にするため "owner_repo_prNumber" の形式にします
        ...
        trap_data = {
            ...
            "score": None,
            "feedback": None,
            "created_at": datetime.utcnow() # 作成日時
        }
...
def score_review(owner: str, repo: str, pr_number: int,):
    ...
    try:
        # ----------------------------------------------------
        # STEP 0: FirestoreからこのPRの罠データ（正解）を取得（新規変更）
        # ----------------------------------------------------
        ...
        #--------------------------------------------------
        # STEP 4: 採点結果をFirestoreにアップデート記録（新規追加）
        # ----------------------------------------------------
        doc_ref.update({
